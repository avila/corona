...
```{r read data}
(df_br <- read_csv("../data/brazil/covid19-481c1e0ed3e946f3b8bb825703e5c52e.csv", 
    col_types = cols(city_ibge_code = col_character(), 
        confirmed = col_integer(), date = col_date(format = "%Y-%m-%d"), 
        deaths = col_integer(), estimated_population_2019 = col_integer())) %>% 
...
```{r figures, fig.height=16, fig.width=12}
# gen data to plot
filter_category <- c("Cum_Cases", "Cum_Deaths")[2]
null_day_threshold <- 1

filter_location <- c("SP", "RJ")
df_to_plot <- df_br %>% rename(Location=state) %>% 
  group_by(Location) %>% 
  filter(
    place_type == "state",
    Category == filter_category,
    value >= null_day_threshold) %>% 
  mutate(date_shift=0:(n()-1)/1) %>% # generate date shifted after null_day_threshold
  ungroup(Location) %>% 
...
```{r}
gg_title      <- "Evolução do COVID-19 por Estados Brasileiros (números absolutos)"
gg_subtitle   <- ""
gg_caption    <- "Código: github.com/avila/corona/Rmd/brazil_io.Rmd. Fonte: https://brasil.io/dataset/covid19/caso"
gg_x_name     <- "Cumulativo casos confirmados | "
...
gen_plot <- function(data, filter_state=NULL,
  ...
  if (proportional) {
    ...
    gg_subtitle <- paste0("Cumulativ", artigo, "s ",  gg_category, "s desde ", null_day_threshold, nr_end, " ", gg_category,
                          " (escala log10 | por 100k habitantes | atualizado ", updated_at, ")")
    ...
    gg_subtitle <- paste0("Cumulativ", artigo, "s ",  gg_category, "s desde ", null_day_threshold, nr_end, " ", gg_category,
                          " (escala log10 | atualizado ", updated_at, ")")
  }
