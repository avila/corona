...
df <- df %>% group_by(countriesAndTerritories) %>% 
...
df$countriesAndTerritories <- str_replace_all(df$countriesAndTerritories, "_", " ")
df <- df %>% mutate(
  Continent=countrycode(countriesAndTerritories,
                        origin = "country.name",
                        destination = "continent"),
  Continent=if_else(countriesAndTerritories=="Kosovo", 
                    "Europe", Continent),
...
gen_df_to_plot <- function(data, filter_location, null_day_threshold, filter_category) {
  ...
  data %>% 
    ...
    ungroup(countriesAndTerritories) %>% 
    arrange(Continent, countriesAndTerritories, date_shift) %>% 
    filter(toupper(countriesAndTerritories) %in% toupper(filter_location) | Continent %in% filter_location)
}
`%!in%` <- Negate(`%in%`)
...
gen_plot <- function(data, filter_location,
  ...
  df_to_plot$countriesAndTerritories_bkg <- df_to_plot$countriesAndTerritories
  ...
  # plot
  df_to_plot %>% 
    ggplot(aes(x=date_shift, y=value, group=countriesAndTerritories)) +
    geom_line(data=df_to_plot[, c("date_shift", "value", "countriesAndTerritories_bkg")],
              aes(x=date_shift, y=value, group=countriesAndTerritories_bkg), 
              color="grey") +
    geom_line(color = emph_color) + geom_point(color = emph_color, size=1/2) +
    facet_wrap(~countriesAndTerritories, ncol = n_col) +
...
top12 <- df %>% filter(Category == "Cum_Deaths") %>% 
  arrange(desc(value)) %>% pull(countriesAndTerritories) %>% unique() %>% head(12)
top12
```
